...
from .opencl import process_edges_opencl
...
class GPUManager:
    ...
    def process_edges(self, gray: NDArray[np.uint8], params: Dict[str, Any]) -> Dict[str, NDArray]:
        ...
        if self._backend == 'cuda':
            ...
            # Process edges and convert iterator to dictionary
            edge_result = next(process_edges_opencl(float_image, threshold))
            return {
    ...
    try:
        with GPUContext() as ctx:
            if params.get('operation') == 'nms':
                return _apply_nms_gpu(params['edge_strength'], params['angle'], params['width'], params['height'])
